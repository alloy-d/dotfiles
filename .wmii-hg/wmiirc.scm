...
(define status-bars
  `((a-music (string-join (shell "ncmpcpp --now-playing"))
             (fork-run (conc wmii-term "-e ncmpcpp")) ; Eval'd on click
...
(wmii:event-handlers-set!
 `(
   ...
   ((key ,modkey "Shift" "Return")
    . ,(lambda _
         (and-let* ((input (wimenu `(""))))
                   (fork-run (conc "chromium 'http://www.yubnub.org/parser/parse?command=" input "'")))))
